...
min_acceptance_score = 0.72
prev_page_total_blocks = -2
...
def get_area(cb: Page_Processor.ContentBlock) -> int:
    x1, y1, x2, y2 = cb.pos
...
class StepProcessor:
    ...
    def init_doc(self,pdf_doc,meta_dict,all_parts_df):
    ...
    '''
    i want to improove some logic, and allow manual enhancments
    so i need to brake process_page for two:
    1) load page - this will load image, break down all artifacts, according to meta file
    and prepare some workable object, that can be displayed visually.
    here is also the place for image manipulations: remove empty, merge, replace. drop by user ?
    
    2) process page. this will take back the object from above, and process it's logic
    
    load page can happen without processing after it. it will be used for displaying the manual,
    and building the meta file, like irelevant pages, and manual noise removal
    
    '''
    def load_page(self,page_index):
        self.current_page = self.pdf_doc[page_index]
        self.page_size = (int(self.current_page.rect.width), int(self.current_page.rect.height))

        # load image. it's used for preview
        pix = self.current_page.get_pixmap(matrix=fitz.Matrix(1, 1))
    ...
    # def prepare_page(self,page_index):
    #
    #     # process page, get steps
    #     page_processor = Page_Processor.PageProcessor(debug_level=0)
    #     page_processor.set_meta(self.meta_dict)
    #     page_processor.process_page(self.pdf_doc, page_index)
    #
    #
    #     self.last_image = page_processor.page_image
    #     self.last_page_processor = page_processor
    #
    #     print ("step_processor: page loaded")
    #
    #     return page_processor
    #     # here we hopefully have all page_processor data loaded.
    #     # but not even steps detected
    #     # maybe we need two keys. each one for each function

    def process_page(self,page_processor):

        global prev_page_total_blocks
        ...
        # # process page, get steps
        # page_processor = Page_Processor.PageProcessor(debug_level=0)
        # self.pp = page_processor
        # page_processor.set_meta(self.meta_dict)
        # page_processor.process_page(self.pdf_doc, page_index)

        # step_blocks = self.__detect_steps_frame(self.last_page_processor)

        step_blocks = self.__detect_steps_frame(page_processor)
        prev_page_total_blocks = len(self.total_step_bocks)
        ...
        try:
            ...
            if step_blocks:
                ...
                for step_block in step_blocks:
                    ...
                    if left_images_count == 0:
                        print ("WARNING: No image left for thew final ")

                    elif left_images_count == 1: # it's the final step image
                        step_block.final_image_block = last_image
                        last_image.used = True
                    else:
                        sorted_blocks = sorted(relevant_images, key=lambda block: (block.used, -get_area(block)))
                        step_block.final_image_block = sorted_blocks[0]
                        sorted_blocks[0].used = True

                        print("WARNING: too much images left ")

                    # save processed block to full blocks list (hopefully we have enough RAM)

                    self.total_step_bocks.append(step_block)

            '''
            so here we have processed all steps. but maybe we have some loan from the previous one
            but how do we know which was the previous ? because a page can contain many steps
            '''

            prev_block = self.total_step_bocks[prev_page_total_blocks-1]

            if prev_block.final_image_block is None:
                print ("we have some loan from prev step")

                # check if there is submodule text (it may or may not exist)
                numbered_sub_module_text_exist = False
                if page_processor.numbered_sub_module_texts:
                    for text in page_processor.numbered_sub_module_texts:
                        if text.used == False:
                            numbered_sub_module_text_exist = True
                            break

                if numbered_sub_module_text_exist:
                    page_processor.numbered_sub_module_texts.sort(key=lambda block: (block.used,int(block.text)))

                    for text in page_processor.numbered_sub_module_texts:
                        if text.used is False:
                            print("we do have some sub step to process: step num: ",text.text)

                             # we create a fake block, to detection function.
                            # actually, we need only the step_frame value
                            ...
                            # for each step area, check if there is numbered sub steps
                            numbered_steps = self.__detect_page_submodule(page_processor, numbered_step_block,
                                                                          page_processor.images_list)

                            # print ("found numbered steps",numbered_steps)
                            prev_block.sub_steps.extend(numbered_steps)

                    # means we do have a loan from prev step, but step is not continues
                    # so, count how many unused images left. if only one - here it is
                    # if more - get the big one (hopefully)
                    #


                sorted_images = sorted(page_processor.images_list, key=lambda block: (block.used, get_area))
                # print ("sorted images: ",sorted_images)
                if sorted_images[0].used == False:
                    # we got the bigest unused image: take it
                    prev_block.final_image_block = sorted_images[0]
                    sorted_images[0].used = True

                if len(sorted_images) > 1 and sorted_images[1].used == False:
                    print ("Warning. we have left with unused images")


            # do, up to here, we have processed steps, and some "loans" from the prev sub step
            counter = 0
            for image in page_processor.images_list:
                if image.used == False:
                    counter += 1
    ...
    def __detect_steps_frame(self,page_processor):

        pdf_x, pdf_y = page_processor.page_size
        step_blocks = []
        ...
        if not frame_relevant_texts: # means no text in frame
            ...
            for step_text in frame_relevant_texts:
                ...
                for image in frame_relevant_images:
                    ...
                    # note. there maybe noise images. so i'm not sure that the closest is the better
                    # maybe the bigger to the right, but not too much
                    # maybe we need to build a search frame. to the next sub step number (like in big steps)
